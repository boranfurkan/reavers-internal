...
interface BurnMintedCaptainHookReturn {
  burnMintedCaptain: (
    mintAddress: string,
...
export const useBurnMintedCaptain = (): BurnMintedCaptainHookReturn => {
  const [isLoading, setIsLoading] = useState(false);
  const { jwtToken } = useAuth();

  const burnMintedCaptain = useCallback(
    async (
      mintAddress: string,
      dynamicWallet: DynamicWallet,
    ): Promise<boolean> => {
      if (!jwtToken) {
        toast.error('Please log in to burn captain');
        return false;
      }
      ...
      try {
        const burnResponse = await fetch(
          `${config.worker_server_url}/nfts/burn-and-withdraw`,
          {
            method: 'POST',
            headers: {
        ...
        // Step 3: Confirm the burn with convert-captain endpoint
        const confirmResponse = await fetch(
          `${config.worker_server_url}/nfts/handle-assets/convert-captain`,
          {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
        ...
        if (!confirmResponse.ok) {
          ...
          throw new Error(errorData.error || 'Failed to confirm captain burn');
        }

        toast.success('Captain successfully burned!');
        return true;
      } catch (error: any) {
        console.error('Error burning minted captain:', error);
        toast.error(error.message || 'Failed to burn captain');
        return false;
      } finally {
        setIsLoading(false);
      }
