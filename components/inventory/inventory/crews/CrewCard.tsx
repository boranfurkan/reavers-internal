...
const CrewCard: React.FC<CrewCardProps> = ({ crew }) => {
  const crewRarity = getLevelRarity(NFTType.CREW, crew.level || 1);

  // Copy handler with toast feedback
  const handleCopy = () =>
    copyToClipboard(crew.mint?.toString() || crew.uid || '', toast.success);

  // Get equipped entity info if the crew is equipped
  const equippedEntity = crew.equippedTo
    ? getEquippedEntityInfo(crew.equippedTo)
    : null;

  return (
    <motion.div
      id={`crew-card-${crew.mint ? crew.mint : crew.uid}`}
      ...
      <div className="relative z-10 flex h-full flex-col">
        {/* Header section with badge and crew name */}
        <div className="mb-3 flex items-center justify-between border-b border-white/30 p-3">
          <motion.div
            ...
            <h3 className="text-left text-lg font-bold leading-tight tracking-wide text-white">
              CREW
            </h3>
          </motion.div>
          <motion.div
            ...
            style={{
              ...getRarityGradient(crewRarity),
              ...getRarityBorderColor(crewRarity),
            }}>
            {crewRarity}
          </motion.div>
        </div>
        ...
        <div className="flex px-3">
          ...
          <motion.div
            ...
            <Image
              src={
                crew.metadata?.image || '/images/mission-modal/crew-nft.webp'
              }
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center">
                <span className="mr-2 max-w-[100px] text-xs text-white">
                  {shortenSolanaAddress(crew.mint || '')}
                </span>
                <motion.button
                  onClick={handleCopy}
