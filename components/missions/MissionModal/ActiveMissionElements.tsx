...
import { ActiveMission, Item } from '../../../lib/types';
...
const StyledItemDiv = styled.div<{ rarity: string }>`
  display: flex;
...
`;
const StyledShipAndCrewDiv = styled.div<{ rarity: string }>`
  position: relative;
  display: flex;
  flex-direction: column;
  align-items: start;
...
const ActiveMissionElement = React.memo(
  function ActiveMissionElement({ mission }: { mission: ActiveMission }) {
    ...
    let itemSlot1: Item | undefined = undefined;
    let itemSlot2: Item | undefined = undefined;
    let crewNft: CrewNFT | undefined = undefined;
    let shipNft: ShipNFT | undefined = undefined;

    if (reaver) {
      [itemSlot1, itemSlot2] = getItemsDataFromNFT(reaver);
      crewNft = nfts.crewsInGame.find(
        (filteredNFT) => filteredNFT.uid === reaver.equippedCrew,
      ) as CrewNFT;
      shipNft = nfts.shipsInGame.find(
        (filteredNFT) => filteredNFT.uid === reaver.equippedShip,
      ) as ShipNFT;
    }
              ...
              <div className="grid h-full w-full grid-cols-2 gap-3">
                {itemSlot1 ? (
                  <StyledItemDiv
                    className="relative"
                    rarity={
                      getLevelRarity(NFTType.ITEM, itemSlot1.level || 1) ||
                      'COMMON'
                    }>
                    <RarityBadge
                      ...
                      secondayText={`(${itemSlot1.level || 1})`}
                      size="extraSmall"
                      className="absolute left-1 top-1 z-50"
                    />
                    <Image
                      src={
                        (itemSlot1 && itemSlot1.itemStats?.image!) ||
                        '/images/weapon-axe.webp'
                      }
                      alt={itemSlot1 ? itemSlot1.name! : 'itemSlot1'}
                      className="mt-3 h-[70px] w-[70px] object-contain transition-all duration-300 ease-in-out hover:scale-110"
                      width={60}
                      height={60}
                      style={{ pointerEvents: 'none' }}
                      unoptimized
                ...
                {itemSlot2 ? (
                  <StyledItemDiv
                    className="relative"
                    rarity={
                      getLevelRarity(NFTType.ITEM, itemSlot2.level || 1) ||
                      'COMMON'
                    }>
                    <RarityBadge
                      bgColor={getRarityColorWithOpacity(
                      ...
                      secondayText={`(${itemSlot2.level || 1})`}
                      size="extraSmall"
                      className="absolute left-1 top-1"
                    />
                    <Image
                      src={
                        (itemSlot2 && itemSlot2.itemStats?.image!) ||
                        '/images/weapon-axe.webp'
                      }
                      alt={itemSlot2 ? itemSlot2.name! : 'itemSlot2'}
                      className="mt-3 h-[70px] w-[70px] object-contain transition-all duration-300 ease-in-out hover:scale-110"
                      width={60}
                      height={60}
                      style={{ pointerEvents: 'none' }}
                ...
                {shipNft ? (
                  <StyledShipAndCrewDiv
                    rarity={getLevelRarity(NFTType.SHIP, shipNft.level || 1)}
                    className="relative col-span-1 overflow-hidden">
                    <RarityBadge
                      bgColor={getRarityColorWithOpacity(
                        getLevelRarity(
                          NFTType.SHIP,
                          shipNft.level ? shipNft.level : 1,
                        ),
                      )}
                      rarityText={getLevelRarity(
                        NFTType.SHIP,
                      ...
                      secondayText={`(${shipNft.level})`}
                      size="extraSmall"
                      className="absolute left-1 top-1 z-10"
                    />
                    <Image
                      src={shipNft.metadata?.image || '/images/reavers.webp'}
                      alt={shipNft.metadata?.name || 'SHIP'}
                      className="w-full rounded-[6px] transition-all duration-300 ease-in-out hover:scale-105"
                      objectFit="cover"
                      style={{ pointerEvents: 'none' }}
                      fill
                      unoptimized
                ...
                {crewNft ? (
                  <StyledShipAndCrewDiv
                    rarity={getLevelRarity(NFTType.CREW, crewNft.level || 1)}
                    className="relative col-span-1 overflow-hidden">
                    <RarityBadge
                      bgColor={getRarityColorWithOpacity(
                        getLevelRarity(
                          NFTType.CREW,
                          crewNft.level ? crewNft.level : 1,
                        ),
                      )}
                      rarityText={getLevelRarity(
                        NFTType.CREW,
                      ...
                      secondayText={`(${crewNft.level})`}
                      size="extraSmall"
                      className="absolute left-1 top-1 z-10"
                    />
                    <Image
                      src={
                        crewNft.metadata?.image ||
                        '/images/mission-modal/crew-nft.webp'
                      }
                      alt={crewNft.metadata?.name || 'CREW'}
                      className="w-full rounded-[6px] transition-all duration-300 ease-in-out hover:scale-105"
                      objectFit="cover"
                      fill
                      style={{ pointerEvents: 'none' }}
  ...
  // Compare props for React.memo
  // (prevProps, nextProps) =>
  // 	prevProps.isSelected == nextProps.isSelected &&
  // 	JSON.stringify(prevProps.mission) == JSON.stringify(nextProps.mission)
);
